...
void Mainwin::toGui(std::string s, int participant, int balance, int pot, int val)
{
	...
	{
	activateExchange(participant, val);
	}
...
void Mainwin::activateExchange(int participant,int turn)
{
	if (turn==1 && participant==1)
	{
	exchange.set_sensitive(true);
	}
        if (turn==2 && participant==2)
        {
	 exchange.set_sensitive(true);
        }
	if (turn==3 && participant==3)
        {
	exchange.set_sensitive(true);
        }
        if (turn==4 && participant==4)
        {
	exchange.set_sensitive(true);
        }
        if (turn==5 && participant==5)
        {
	exchange.set_sensitive(true);
	}
}
...
{
potLabel.set_markup("<span size='10000' color ='red' weight='bold'>Player "  + to_string(val)+" takes the pot!"+"</span>");
