	...
	*/
    ...
    void chat_client::do_read_body()
    {
		...
		if (from_dealer["action"].empty()==false)
		{
			...
			{
                                win->toGui("updateCallAction",from_dealer["participant"], 0,0,0);
			}if(from_dealer["action"]=="bet")
			{
                                win->toGui("updateBetAction",from_dealer["participant"], from_dealer["balance"], from_dealer["pot"], from_dealer["bid"]);
			}if(from_dealer["action"]=="raised")
			{
                                win->toGui("updateRaiseAction",from_dealer["participant"], 0,0,0);
		...
		{
			if (newRound)win->toGui("updatePot",from_dealer["participant"], from_dealer["balance"],from_dealer["pot"],0);
			...
			if (turn>0 && playerNo>0 && turn==playerNo && !skipStatus)
			{
				...
				{
				win->toGui("updateCallRaiseButtons",0, 0,0,0);
				}
				//win->toGui("updateBalances",0,0); doesnt work
			}
