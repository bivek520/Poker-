...
//            cout<<"this is the write"<<endl;
    ...
    void chat_client::do_read_body()
    {
		...
		if (from_dealer["action"].empty()==false)
		{
			...
			{
                                win->toGui("updateCallAction",from_dealer["participant"], 0,0,from_dealer["bid"]);
		...
		if(turn>0 && playerNo>0 && from_dealer["getAni"].empty())
		{
			...
			if (from_dealer["winner"].empty()==false)
			{
				...
				//win->toGui("updateVals",from_dealer["participant"], from_dealer["winnerBalance"],from_dealer["pot"],0);
				win->displayWinner(from_dealer["winner"],from_dealer["pot"]);
				//int tempPot = from_dealer["pot"];
				//if(playerNo==from_dealer["winner"])balance+=tempPot;
				if (from_dealer["getAni"].empty()==false)win->toGui("takeAni",0,0,0,0);
				/*
				if(balance < 5)
				{
					cout<<to_string(playerNo)+" BUSTED OUT"<<endl;
					//win->toGui("bustOut",playerNumber,0,0,0);
				}
				*/
			}
